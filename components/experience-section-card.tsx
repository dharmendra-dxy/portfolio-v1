...
import { ExperienceItem } from "./sections/experience-section";
  ...
  return (
    ...
    >
      ...
      >
        <Building2
          size={24}
          className={`${isHovered ? "text-red-400" : "text-white"}  md:w-6 md:h-6`}
        />
      </motion.div>
      ...
      >
        ...
        <div className="mb-4">
          {exp.url && (
            <Image
              ...
              height={20}
              width={70}
              className="not-target:object-cover rounded-sm  overflow-hidden hover:scale-105 duration-300"
            />
          )}
        </div>
        ...
        <div className="space-y-3">
          ...
          {initialDescriptions.map(
            (desc: ExperienceDescription, index: number) => (
              <motion.div
                key={index}
          ...
          {hasMoreDescriptions && (
            ...
            >
              <span className="text-sm font-medium">
                {isExpanded
                  ? "View Less"
                  : `View More (${remainingDescriptions.length} more)`}
              </span>
              <motion.div
              ...
              >
                <ChevronsDown
                  size={16}
                  className="group-hover:transform group-hover:scale-110 transition-transform"
                />
              </motion.div>
            </motion.button>
          )}
